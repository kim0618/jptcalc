...
- **[지정 2026-07-13] 셀프등기 절차 1편 (부동산·registry 클러스터 증축)** — 7/6~7/12 리포트 근거: registry-cost-guide.html이 네이버 모바일검색 유입 11→43(4배)로 터지며 등기 클러스터가 이번주 최대 성과. 기존 등기비용 글은 "비용" 앵글이므로 중복 회피를 위해 **"셀프등기 절차(how-to)" 앵글**로 작성한다 - 인터넷등기소(등기소 방문 없이) 소유권이전등기 직접 하는 순서·필요서류·법무사 비용 절감액 중심. **작성 전 필수 가드**: registry-cost-guide.html의 h2를 전수 확인해 절차 관련 앵글이 이미 있는지 교차 확인(근저당·소유권이전등기 개념은 이미 커버됨), 근중복이면 앵글 재조정하거나 미작성 판단. `/calc/realestate/registry/`에 CTA·relatedCalc·detail-shell guides 양방향 연결. 완료 시 이 항목 `[완료]` 표기.
