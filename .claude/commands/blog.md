...
### 텍스트 분량
- HTML 태그·CSS·스크립트를 제외한 **순수 텍스트 2,500자 이상** 작성
- 2,000자 미만은 얇은 글로 간주해 재작성
...
**링크 형식:**
```html
<p style="font-size:14px;margin:16px 0;">👨‍👩‍👧 부모님께도 알려드리세요 - <a href="https://www.bumohyetaek.kr/articles/[카테고리]/[슬러그]" style="color:[--primary색상];">[부모혜택 글 제목]</a></p>
```
...
3. h2/h3 섹션 + 데이터 비교표
4. 인물·상황 기반 계산 예시 (highlight-box 활용)
5. 본문 중간 인라인 계산기 링크
6. 추가 h2 섹션 + 두 번째 비교표
7. **같은 카테고리 기존 글 교차 링크** (문맥 내 자연 삽입)
8. cta-box (관련 계산기 버튼)
9. 관련 계산기 링크 p태그 (style="font-size:13px;color:#6B7280;margin-top:8px;")
10. faq-item 4개
11. 마무리 h2 단락
...
### category 값 매핑
- 부동산=realestate, 세금=tax, 금융=finance, 연봉=salary, 건강=health, 반려동물=pet, 날짜·D-day=date, AI·테크=ai
...
- 카테고리별 tag 클래스: 부동산=tag-realestate / 세금=tag-tax / 금융=tag-finance / 연봉=tag-salary / 건강=tag-health / 반려동물=tag-pet / 날짜·D-day=tag-date / AI·테크=tag-ai
...
```
검증 항목:
□ HTML 태그 제거 후 순수 텍스트 2,500자 이상인지 확인
  - Bash: cat 파일.html | sed 's/<[^>]*>//g' | tr -s ' \n' | wc -c
□ 계산 예시 3개 이상, 수식 검산 완료
□ 비교표 2개 이상 (실제 수치 포함)
