...
- 이자·저축 세율 구분 (혼동 주의):
  ...
  - 조합 예탁금 비과세(새마을금고·농협·신협·수협 등, 3,000만원 한도): 실효세율 ~1.4% (농특세만)
  - ISA 비과세 초과분 분리과세: 9.9% (소득세 9% + 지방소득세 0.9%)
  - "세금우대 9.5%"는 2015년 폐지된 세금우대종합저축 세율 → 현행 글에서 사용 시 오류
...
- **특히 주의 - BreadcrumbList position 3**: item URL이 `/calc/` 경로면 오류. 반드시 `/blog/?cat=카테고리명` 형식이어야 함 (반복 발견되는 패턴)
